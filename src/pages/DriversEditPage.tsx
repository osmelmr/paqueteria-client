...
export default function DriversEditPage() {
  const { id } = useParams();
  ...
  if (!driver) {
    return <div className="max-w-7xl mx-auto w-full min-w-0"><div className="p-[18px] border border-border rounded-xl bg-surface shadow-lg mb-[18px]">Chofer no encontrado</div></div>;
  }
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setLocalError(null);
    try {
      await updateDriver.mutateAsync({ id: id!, dto: { name } });
      navigate('/drivers');
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full min-w-0">
      <div className="p-[18px] border border-border rounded-xl bg-surface shadow-lg mb-[18px]">
        <h2 className="text-gray-900 dark:text-gray-100 font-semibold m-0 mb-4">Editar chofer</h2>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3.5">
          ...
          <label className="flex flex-col gap-1.5 font-medium">
            Vehiculos asignados
            <input className="border border-border rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200" value={driver.vehicles?.map((v) => v.vehicle.name).join(', ') || 'Ninguno'} readOnly disabled />
          </label>
          <div className="flex gap-2.5 flex-wrap mt-3.5" style={{ gridColumn: '1 / -1' }}>
            <button type="submit" className="bg-purple-500 dark:bg-purple-400 text-white font-semibold rounded-xl px-4 py-3 text-sm cursor-pointer border-none hover:bg-purple-600 dark:hover:bg-purple-500 transition-colors disabled:opacity-50" disabled={updateDriver.isPending}>Guardar</button>
