    ...
    <div className="max-w-7xl mx-auto w-full min-w-0">
      <div className="p-[18px] border border-gray-200 dark:border-gray-700 rounded-xl bg-[#dbdbdb] dark:bg-[#1e1f27] shadow-lg mb-[18px]">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h2 className="text-gray-900 dark:text-gray-100 font-semibold m-0">Agencias</h2>
          <button type="button" className="bg-purple-500 dark:bg-purple-400 text-white font-semibold rounded-xl px-4 py-3 text-sm cursor-pointer border-none hover:bg-purple-600 dark:hover:bg-purple-500 transition-colors" onClick={() => navigate('/agencies/new')}>Nueva agencia</button>
        </div>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="border border-gray-200 dark:border-gray-700 p-2.5 text-gray-700 dark:text-gray-300">
                    <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded-full ${item.type === 'MARITIMA' ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300' : 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300'}`}>
                      {item.type === 'MARITIMA' ? 'Marítima' : 'Aérea'}
                    </span>
                  </td>
                  <td className="border border-gray-200 dark:border-gray-700 p-2.5 text-gray-700 dark:text-gray-300">
                    <div className="flex gap-2 flex-wrap">
                      <button type="button" className="bg-purple-500 dark:bg-purple-400 text-white rounded-xl px-2.5 py-2 text-xs cursor-pointer border-none hover:bg-purple-600 dark:hover:bg-purple-500 transition-colors" onClick={() => navigate(`/agencies/${item.id}/edit`)}>Editar</button>
              ...
              {items.length === 0 && !isLoading && <tr><td colSpan={4} className="border border-gray-200 dark:border-gray-700 p-2.5 text-gray-700 dark:text-gray-300" style={{ textAlign: 'center', color: 'var(--muted)' }}>No hay agencias registradas</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
