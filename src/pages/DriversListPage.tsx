...
export default function DriversListPage() {
  const { data: items = [], isLoading, error: queryError } = useDrivers();
  const deleteEntity = useDeleteDriver();
    ...
    <div className="max-w-7xl mx-auto w-full min-w-0">
      <div className="p-[18px] border border-border rounded-xl bg-surface shadow-lg mb-[18px]">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h2 className="text-gray-900 dark:text-gray-100 font-semibold m-0">Choferes</h2>
          <button type="button" className="bg-purple-500 dark:bg-purple-400 text-white font-semibold rounded-xl px-4 py-3 text-sm cursor-pointer border-none hover:bg-purple-600 dark:hover:bg-purple-500 transition-colors" onClick={() => navigate('/drivers/new')}>Nuevo chofer</button>
        </div>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="border border-border p-2.5 text-gray-700 dark:text-gray-300">
                    {item.vehicles?.length ? item.vehicles.map((v) => v.vehicle.name).join(', ') : '-'}
                  </td>
                  <td className="border border-border p-2.5 text-gray-700 dark:text-gray-300">
                    <div className="flex gap-2 flex-wrap">
                      <button type="button" className="bg-purple-500 dark:bg-purple-400 text-white rounded-xl px-2.5 py-2 text-xs cursor-pointer border-none hover:bg-purple-600 dark:hover:bg-purple-500 transition-colors" onClick={() => navigate(`/drivers/${item.id}/edit`)}>Editar</button>
              ...
              {items.length === 0 && !isLoading && <tr><td colSpan={3} className="border border-border p-2.5 text-gray-700 dark:text-gray-300" style={{ textAlign: 'center', color: 'var(--muted)' }}>No hay choferes registrados</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
