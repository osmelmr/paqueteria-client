...
import type { Agency } from '../api/agencies.api';
...
function AgencyEditForm({ item }: { item: Agency }) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setError(null);
    try {
      await updateEntity.mutateAsync({ id: item.id, dto: { name } });
      navigate('/agencies');
    } catch (err) {
      ...
      <div className="p-[18px] border border-border rounded-xl bg-surface shadow-lg mb-[18px]">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3.5 items-end">
          ...
          <div className="flex gap-2.5 flex-wrap mt-3.5">
            ...
            <button type="button" className="bg-slate-50 dark:bg-slate-800 text-gray-900 dark:text-gray-100 border border-border font-semibold rounded-xl px-4 py-3 text-sm cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors" onClick={() => navigate('/agencies')}>Cancelar</button>
          </div>
        </form>
      </div>
    </div>
