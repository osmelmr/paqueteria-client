    ...
    <div className="page">
      <div className="panel">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h2 style={{ margin: 0 }}>Municipios</h2>
          <button type="button" onClick={() => navigate('/municipes/new')}>Nuevo municipio</button>
        </div>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div className="inline-actions">
                      <button type="button" className="small" onClick={() => navigate(`/municipes/${item.id}/edit`)}>Editar</button>
                      <button type="button" className="small secondary" onClick={() => handleDelete(item.id)}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
              {items.length === 0 && !isLoading && <tr><td colSpan={3} style={{ textAlign: 'center', color: 'var(--muted)' }}>No hay municipios registrados</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
